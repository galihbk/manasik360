...
export default function NewBlogPage() {
  ...
  const handleSaveBlog = async (e: React.FormEvent) => {
    ...
    // Prepend all images markdown to content
    const finalContent = images.length > 0
      ? images.map(img => `![cover](${img})`).join('\n') + '\n\n' + blogContent
      : blogContent;

    try {
      const res = await client.createSuperAdminBlog({
        title: blogTitle,
      ...
      setError('Terjadi kesalahan koneksi saat menyimpan artikel.');
    } finally {
    ...
    <div className="p-6 md:p-8 w-full bg-slate-50 min-h-screen">
      <div className="w-full space-y-6">
        ...
        <div className="border-b border-slate-200 pb-5">
          <h1 className="text-2xl font-extrabold text-slate-900 mt-2">Tulis Artikel Baru</h1>
          <p className="text-xs text-slate-400 mt-1">Unggah beberapa gambar pendukung sekaligus. Slider kartu preview akan aktif otomatis.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <form onSubmit={handleSaveBlog} className="lg:col-span-7 bg-white border border-slate-200 rounded-xl shadow-sm p-6 space-y-6">
            ...
            <div className="pt-5 border-t border-slate-100 flex justify-end gap-2 text-xs font-bold">
              ...
              >
                {actionLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : 'Simpan & Publikasikan'}
              </button>
            </div>
          </form>
