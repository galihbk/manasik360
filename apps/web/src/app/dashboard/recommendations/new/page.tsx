...
export default function NewRecommendationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await client.createRecommendation({
        ...formData,
        imageUrl: images.join(',') // Save multiple images as comma separated list
      });
      if (res && res.success) {
        showToast('Rekomendasi berhasil dibuat!');
        setTimeout(() => {
          router.push('/dashboard/recommendations');
        }, 1000);
      } else {
        showToast(res.message || 'Gagal membuat rekomendasi', 'error');
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="p-6 md:p-8 w-full bg-slate-50 min-h-screen">
      ...
      <div className="w-full space-y-6">
        ...
        <div className="border-b border-slate-200 pb-5">
          <h1 className="text-2xl font-extrabold text-slate-900 mt-2">Tambah Rekomendasi Baru</h1>
          <p className="text-xs text-slate-400 mt-1">Formulir kemitraan hotel/souvenir dengan fitur upload galeri foto dan pratinjau real-time.</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="lg:col-span-7 bg-white border border-slate-200 rounded-xl shadow-sm p-6 space-y-6 text-xs font-bold text-slate-700">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
                  <span className="block uppercase tracking-widest text-[9px] text-slate-400 font-black border-b border-slate-200 pb-1">Target Halaman</span>
                  <div className="flex flex-col gap-2">
                    <label className="flex items-center gap-2 cursor-pointer select-none">
                      <input
                        type="checkbox"
              ...
              <div>
                <div className="relative h-44 bg-slate-100 flex items-center justify-center group/preview">
                  {images.length > 0 ? (
                    ...
                    <div className="text-slate-350 flex flex-col items-center justify-center gap-1">
                ...
                <div className="p-5 space-y-2.5">
                  ...
                  <p className="text-xs text-slate-505 leading-normal line-clamp-3">
