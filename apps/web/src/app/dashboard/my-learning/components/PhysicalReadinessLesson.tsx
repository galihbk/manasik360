...
export default function PhysicalReadinessLesson({
  activeTab,
  lang,
  quizAnswers,
  setQuizAnswers,
  ...
  const currentQuiz = sd.lesson_1_1_quiz || [];
  const markdownContent: string = sd.lesson_1_1_content || '';
          ...
          <div className="space-y-6">
            <div>
              <h3 className="text-2xl font-extrabold text-slate-900 tracking-tight">
                {lang === 'id' ? 'Protokol Kesiapan Fisik' : lang === 'ar' ? 'بروتوكولات الاستعداد البدني' : 'Physical Readiness Protocols'}
              </h3>
              <p className="text-xs text-slate-400 font-bold mt-1.5 flex items-center gap-3">
                <span className="flex items-center gap-1.5">
                  <Clock className="w-3.5 h-3.5 text-slate-450" />
                  <span>15 {sd.minutes}</span>
                </span>
                <span>•</span>
                <span className="flex items-center gap-1.5">
